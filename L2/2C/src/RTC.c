...
void RTC_Init(void) {
	...
	RTC_Set_Calendar_Date(RTC_WEEKDAY_WEDNESDAY, 15, RTC_MONTH_APRIL, 26); /* [TODO] These values are stubs - fill in current date */
	
	// Configure the Time 
	RTC_Set_Time(RTC_TR_PM, 9, 30, 0); /* [TODO] These values are stubs - fill in current time */
...
void RTC_Set_Calendar_Date(uint32_t WeekDay, uint32_t Day, uint32_t Month, uint32_t Year) {
	...
	uint8_t d = __RTC_CONVERT_BIN2BCD(Day);
	uint8_t m = __RTC_CONVERT_BIN2BCD(Month);
	uint8_t y = __RTC_CONVERT_BIN2BCD(Year);
	
	dr |= (WeekDay << RTC_POSITION_DR_WDU);
	
	dr |= ((d & 0xF0) << (RTC_POSITION_DR_DT - 4));
	dr |= ((d & 0x0F) << RTC_POSITION_DR_DU);
	
	dr |= ((m & 0xF0) << (RTC_POSITION_DR_MT - 4));
	dr |= ((m & 0x0F) << RTC_POSITION_DR_MU);
	
	dr |= ((y & 0xF0) << (RTC_POSITION_DR_YT - 4));
	dr |= ((y & 0x0F) << RTC_POSITION_DR_YU);
...
void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second) {
	...
	uint8_t h = __RTC_CONVERT_BIN2BCD(Hour);
	uint8_t m = __RTC_CONVERT_BIN2BCD(Minute);
	uint8_t s = __RTC_CONVERT_BIN2BCD(Second);
	
	tr |= ((h & 0xF0) << (RTC_POSITION_TR_HT - 4));
	tr |= ((h & 0x0F) << RTC_POSITION_TR_HU);
	
	tr |= ((m & 0xF0) << (RTC_POSITION_TR_MT - 4));
	tr |= ((m & 0x0F) << RTC_POSITION_TR_MU);
	
	tr |= ((s & 0xF0) << (RTC_POSITION_TR_ST - 4));
	tr |= ((s & 0x0F) << RTC_POSITION_TR_SU);
...
uint32_t RTC_TIME_GetHour(void) {
	uint32_t ht = (RTC->TR & RTC_TR_HT) >> RTC_POSITION_TR_HT;
	uint32_t hu = (RTC->TR & RTC_TR_HU) >> RTC_POSITION_TR_HU;
	return (ht << 4) | hu;
}

uint32_t RTC_TIME_GetMinute(void) {
	uint32_t mt = (RTC->TR & RTC_TR_MNT) >> RTC_POSITION_TR_MT;
	uint32_t mu = (RTC->TR & RTC_TR_MNU) >> RTC_POSITION_TR_MU;
	return (mt << 4) | mu;
}

uint32_t RTC_TIME_GetSecond(void) {
	uint32_t st = (RTC->TR & RTC_TR_ST) >> RTC_POSITION_TR_ST;
	uint32_t su = (RTC->TR & RTC_TR_SU) >> RTC_POSITION_TR_SU;
	return (st << 4) | su;
}

uint32_t RTC_DATE_GetMonth(void) {
	uint32_t mt = (RTC->DR & RTC_DR_MT) >> RTC_POSITION_DR_MT;
	uint32_t mu = (RTC->DR & RTC_DR_MU) >> RTC_POSITION_DR_MU;
	return (mt << 4) | mu;
}

uint32_t RTC_DATE_GetDay(void) {
	uint32_t dt = (RTC->DR & RTC_DR_DT) >> RTC_POSITION_DR_DT;
	uint32_t du = (RTC->DR & RTC_DR_DU) >> RTC_POSITION_DR_DU;
	return (dt << 4) | du;
}

uint32_t RTC_DATE_GetYear(void) {
	uint32_t yt = (RTC->DR & RTC_DR_YT) >> RTC_POSITION_DR_YT;
	uint32_t yu = (RTC->DR & RTC_DR_YU) >> RTC_POSITION_DR_YU;
	return (yt << 4) | yu;
}
